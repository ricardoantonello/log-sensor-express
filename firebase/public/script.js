...
function getParameterByName (name){
     ...
     * This reference comes with a listener to read the value for the first time, and execute some action
     * everytime a value is received
     */
    const temperatureListener = database.ref('temperature')

    temperatureListener.on('value', data =>{
        /**
         * The contents of the listener are pretty much the same as the listeners in our previous
         * chapters. The only     difference being that the value of the data being read has to be
         * accessed through     the "val" getter method, rather than the data.value attribute
         */
        const now = new Date()
